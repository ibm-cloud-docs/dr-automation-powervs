...
The solution minimizes manual intervention by automating DR workflows by using tools like the DR Service Broker and the KSYS orchestrator. These components synchronize resources, manage failover priorities, and optimize RPOs and RTOs, ensuring high availability and business continuity.
...
Before implementing the VM Recovery Manager (VMRM) DR solution, ensure that you have the following resources planned and ready:  
...
7. If VSI has multiple IP's and you can configure `squid.conf` with different IP which can be seen in /etc/squid/squid.conf with the following entry:
   `http_port IP:3128`
Here IP is used as a proxy IP in squid configuration. 

To enable communication to external services, enable following variables:
